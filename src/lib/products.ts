import { fetchWithTimeout } from "src/helpers/fetchWithTimeout";
...
const siteApiKey =
  process.env.NEXT_PUBLIC_SITE_API_KEY || process.env.SITE_API_KEY;
...
export function productLink(
  product: Pick<Product, "article" | "path">,
): string {
...
function detectAvailabilityStatus(
  raw: string,
): AvailabilityState["status"] | null {
  if (!raw) return null;
  if (
    /(немає|нет|відсут|out of stock|out_of_stock|outofstock|sold out|no stock)/i.test(
      raw,
    )
  ) {
    return "out_of_stock";
  ...
  if (
    /(під замовлення|под заказ|preorder|pre-order|on order|backorder|back order|made to order)/i.test(
      raw,
    )
  ) {
    return "on_order";
