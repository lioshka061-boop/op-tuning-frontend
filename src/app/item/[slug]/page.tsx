...
import { ProductPageLayout } from "src/components/ProductPageLayout";
import {
  loadProduct,
  loadProducts,
  plainText,
...
import { productYears } from "src/lib/seo";
...
const resolveProduct = cache(async (paramsSlug: string) => {
  ...
  const articleCandidates = buildArticleCandidates(paramsSlug).filter(
    (candidate) =>
      !articleFromSlug ||
      candidate.toLowerCase() !== articleFromSlug.toLowerCase(),
  );
  ...
  if (articleFromSlug) {
    match = searchPool.find(
      (p) => p.article.toLowerCase() === articleFromSlug.toLowerCase(),
    );
  }
  if (!match) {
  ...
  if (!match) {
    match = searchPool.find((p) =>
      decodedLower.endsWith(`-${p.article.toLowerCase()}`),
    );
  }
  if (!match) {
    match = searchPool.find(
      (p: Product) => p.slug && p.slug.toLowerCase() === decodedLower,
    );
  }
...
function getRecommendedProducts(
  ...
  const sameBrand = (p: CatalogProduct) =>
    slugify(p.brand) === slugify(current.brand) ||
    p.brand.toLowerCase() === current.brand.toLowerCase();
  const sameModel = (p: CatalogProduct) =>
    slugify(p.model) === slugify(current.model) ||
    p.model.toLowerCase() === current.model.toLowerCase();
  ...
  const filtered =
    sameModelCategory.length > 0
      ? sameModelCategory
      : sameModelItems.length > 0
        ? sameModelItems
        : [];
  ...
  return {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Каталог", item: "/catalog" },
      {
        ...
        position: 3,
        name: "Марка",
        item: `/catalog/${brandSlug}`,
      },
      {
        "@type": "ListItem",
        position: 4,
        name: "Модель",
        item: `/catalog/${brandSlug}/${modelSlug}`,
      },
...
export async function generateMetadata({
  ...
  const { product } = data;
  const title =
    product.meta_title || product.og_title || product.title || "Товар";
  const description =
  ...
  const images = product.og_image
    ? [product.og_image]
    : product.images?.filter(Boolean) || [];
  const robotsValue =
    product.robots || (indexable ? "index,follow" : "noindex,follow");
...
export default async function ItemPage({
  ...
  const faqSchema =
    indexable && product.faq ? faqSchemaFromHtml(product.faq, canonical) : null;
