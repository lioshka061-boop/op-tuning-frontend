import "../globals.css";
import { loadSeoPages } from "src/lib/seo-pages";
import { plainText } from "src/lib/products";
...
export default async function SeoPagesIndex() {
  ...
  const accessories = items.filter(
    (item) => item.page_type === "accessories_car",
  );
                ...
                <div className="article-grid">
                  {tuning.map((item) => (
                    <article key={item.id}>
                      ...
                      <p>
                        {excerpt(
                          item.meta_description || item.seo_text || "",
                        ) || "Опис готується."}
                      </p>
                    </article>
                  ))}
