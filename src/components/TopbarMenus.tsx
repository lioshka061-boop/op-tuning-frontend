...
import { formatPrice, productLink } from "src/lib/products";
import {
  getCart,
  removeFromCart,
  incrementCartItem,
...
function CartMenu() {
  ...
  return (
    <div
      className={`top-action ${items.length > 0 ? "has-items" : ""}`}
      ref={ref}
    >
      <button
        type="button"
...
function FavoritesMenu() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("favorites:update", handler as EventListener);
  }, [refresh]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span>Обране</span>
        {items.length > 0 && (
          <span className="action-badge">{items.length}</span>
        )}
      </button>
      {open && (
        <div className="action-dropdown">
...
function GarageMenu() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("garage:update", handler as EventListener);
  }, [refresh]);
          ...
          {items.length === 0 ? (
            ...
            <div className="dropdown-list">
              {items.map((item, idx) => {
                ...
                const href = modelSlug
                  ? `/catalog/${brandSlug}/${modelSlug}`
                  : `/catalog/${brandSlug}`;
                return (
                  <div
