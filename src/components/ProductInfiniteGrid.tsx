...
import type { LoadProductsParams, Product } from "src/lib/products";
import {
  formatPrice,
...
export function ProductInfiniteGrid({
  ...
  const [hasMore, setHasMore] = useState(
    initialOffset + initialItems.length < total,
  );
  ...
  const loadMore = useCallback(() => {
    ...
    fetchProductsClient({ ...fetchParams, offset })
      .then(({ items: next, total: nextTotal }) => {
        ...
        setItems((prev) =>
          sortByAvailability(uniqueByArticle([...prev, ...next])),
        );
        setOffset((prev) => {
          ...
          <article className="product-card" key={p.article}>
            ...
            <div className="price-row">
              <div className="price-stack">
                ...
                {installment && (
                  <span className="price-installment">{installment}</span>
                )}
              </div>
