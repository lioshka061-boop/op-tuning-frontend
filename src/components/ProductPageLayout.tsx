...
import { formatPrice as formatCatalogPrice, plainText } from "src/lib/products";
...
import { ItemTabs } from "src/app/item/[slug]/Tabs";
...
export function ProductPageLayout({ product, recommendations }: Props) {
  ...
  const tabs = [
    ...
    },
    {
      ...
      label: "Відгуки",
      content: (
        <ReviewsSection productKey={product.article} isMaxton={isMaxton} />
      ),
    },
    {
      id: "faq",
      ...
      <section className={styles.layout}>
        ...
        <aside className={styles.right}>
          <div className={`${styles.infoBlock} info card`}>
            <h1>{title}</h1>
            {product.article && (
              <p className="meta meta-article">Артикул: {product.article}</p>
            )}
            {isMaxton && (
            ...
            <div className="price-line">
              <div className={`price ${product.price ? "" : "muted"}`}>
          ...
          <div className={`${styles.badgeBlock} info-badges`}>
            ...
            <div className="info-badge">
              ...
              <div className="info-badge-sub">
                Захищені транзакції та перевірені сервіси
              </div>
            </div>
          </div>
          <div className={`${styles.installmentBlock} installment-box card`}>
            ...
            <div className="installment-body">
              <div className="installment-highlight">
                72% клієнтів обрали саме цей платіж
              </div>
              <div className="installment-logos" aria-hidden="true">
                <img
                  src="/installments/a-bank.jpg"
                  alt=""
                  width={32}
                  height={32}
                />
                <img
                  src="/installments/pumb.svg"
                  alt=""
                  width={32}
                  height={32}
                />
                <img
                  src="/installments/mono-lapka(1).png"
                  alt=""
                  width={32}
                  height={32}
                />
              ...
              {installment ? (
                <>
                  <div className="installment-monthly">
                    {formatCatalogPrice(installment.monthly)}{" "}
                    <span>/ місяць</span>
                  </div>
                ...
                <div className="installment-sub">
                  Уточнюйте ціну для розрахунку платежів.
                </div>
              )}
            </div>
          </div>
