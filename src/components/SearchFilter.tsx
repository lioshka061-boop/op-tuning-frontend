...
export function SearchFilter({
  ...
  const [carCategories, setCarCategories] = useState<CategoryNode[]>(
    initialCarCats || [],
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      hasProductCats
        ? Promise.resolve(productCategories)
        : loadProductCategories(),
    ])
      .then(([cars, products]) => {
  ...
  const submit = () => {
    if (variant === "compact") {
      ...
      if (brand && model) {
        ...
        router.push(
          `/catalog/${slugify(brand)}/${slugify(model)}${suffix ? `?${suffix}` : ""}`,
        );
        return;
      }
      if (brand) {
      ...
      if (productCategory) {
        router.push(
          `/catalog?pcat=${encodeURIComponent(slugify(productCategory))}`,
        );
        return;
      }
      router.push("/catalog");
  ...
  const saveToGarage = () => {
    ...
    try {
      ...
      const parsed: Array<{ brand: string; model?: string }> = stored
        ? JSON.parse(stored)
        : [];
      ...
      const unique = parsed
        ...
        .filter(
          (c, idx, arr) =>
            arr.findIndex(
              (x) =>
                slugify(x.brand) === slugify(c.brand) &&
                slugify(x.model || "") === slugify(c.model || ""),
            ) === idx,
        );
  ...
  return (
    <form
      ...
      id="garage"
    >
      <label className="visually-hidden" htmlFor={`brand-${variant}`}>
        Марка
      </label>
      <select
        id={`brand-${variant}`}
      ...
      <label className="visually-hidden" htmlFor={`model-${variant}`}>
        Модель
      </label>
      <select
        id={`model-${variant}`}
      ...
      <label className="visually-hidden" htmlFor={`pcat-${variant}`}>
        Категорія
      </label>
      <select
        id={`pcat-${variant}`}
